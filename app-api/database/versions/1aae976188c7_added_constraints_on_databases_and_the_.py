...
def upgrade() -> None:
    ...
    op.execute("INSERT INTO users (id, username, image_url_hash) (select user_id, 'unknown user' || ROW_NUMBER() OVER (ORDER BY user_id) , '0000' from (select distinct user_id from datasets) where user_id not in (select id from users))")
