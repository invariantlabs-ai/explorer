...
function DeletePolicyModalContent(props) {
    ...
    return <div className='form'>
        <h2>Are you sure you want to delete Policy: <b>{props.policy.name}</b>?<br /><br />
...
function MutatePolicyModalContent(props) {
    ...
    const [name, setName] = React.useState(props.policy ? props.policy.name : null);
    const [policyCode, setPolicyCode] = React.useState(action == "update" ? props.policy.content : defaultPolicyCode);
    const [loading, setLoading] = React.useState(false);
    const [error, setError] = React.useState('');
    ...
    const onMutate = () => {
        setLoading(true)
        fetch(action == "update" ? `/api/v1/dataset/${props.dataset_id}/policy/${props.policy.id}` : `/api/v1/dataset/${props.dataset_id}/policy`, {
    ...
    return <div className='policy-editor-form'>
        <label>Name</label>
        <input type="text" value={name || ''} onChange={e => setName(e.target.value)} placeholder="Policy Name" />
        <label>Policy Code</label>
        <Editor height="60%" width="100%" className="policy-editor" defaultLanguage="python" value={policyCode} onChange={(value?: string) => setPolicyCode(value || "")} theme="vs-light" />
        <button aria-label={action} className='primary' disabled={loading || !name || !policyCode || !policyCode.trim()} onClick={onMutate}>
...
export function PoliciesView(props) {
    ...
    const [selectedPolicyForDeletion, setSelectedPolicyForDeletion] = React.useState(null);
    // tracks the policy to be updated.
    const [selectedPolicyForUpdation, setSelectedPolicyForUpdation] = React.useState(null);
                ...
                {dataset.extra_metadata?.policies && dataset.extra_metadata.policies.length > 0 && dataset.extra_metadata.policies.map(policy => {
                    return <div key={policy.id}>
                        <div className='box full setting'>
                            ...
                            <button aria-label="delete" className='danger' onClick={() => setSelectedPolicyForDeletion(policy)}>
                                <BsTrash /> Delete
                            </button>
