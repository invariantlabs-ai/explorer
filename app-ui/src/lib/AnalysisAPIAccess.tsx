...
export function AnalysisConfigEditor(props: {collapsed?: boolean}) {
    ...
    const [advancedMode, _setAdvancedMode] = React.useState(localStorage.getItem("invariantlabs-analysis-config-advanced-mode") === "true");

    const setAdvancedMode = (mode: boolean) => {
        localStorage.setItem("invariantlabs-analysis-config-advanced-mode", mode ? "true" : "false");
    ...
    return <div className="analysis-config-editor">
        <header>
            <InvariantIcon/>
            <h4>
                {collapsed ? <>This feature requires advanced API access.</> : <>Advanced API Access</>}
            </h4>
            {!collapsed && <button className={"button inline toggleable " + (advancedMode ? "toggled" : "")} onClick={() => {
                setAdvancedMode(!advancedMode);
            }}>
                More Settings
            </button>}
            {collapsed && <button className="button inline toggleable" onClick={() => {
        ...
        <div className="info secondary">
            The Analysis API gives you advanced functionality to analyze your agent data. Note that you may need to first be unlocked for access to the Analysis API. If you are not, please <a  href="mailto:model@invariantlabs.ai" target="_blank" rel="noreferrer">contact us</a>.
...
export function useAnalysisConfig(): [
    ...
    const [analysisConfig, setAnalysisConfigState] = React.useState(getAnalysisConfig());
