...
export class Reveal {
  ...
  static create(
    ...
    window["ACTIVE_REVEAL"] = new Reveal(
      id,
      ids,
      segments,
      hash,
...
export function AnchorDiv(props: any & AnchorDivProps) {
  ...
  useEffect(() => {
    ...
    if (ref.current) {
      addAnchorRevealListener(
        ref.current,
        (segments: any[], operation: string) => {
          if (onReveal) onReveal(segments, operation);
        },
      );
      if (afterReveal) {
        addAnchorAfterRevealListener(
          ref.current,
          (segments: any[], operation: string, leafElement: HTMLElement) => {
  ...
  return (
    <div id={id} {...rest} ref={ref} className={className} onClick={onClick}>
      {props.children}
    </div>
  );
}

export function addAnchorRevealListener(
  element: HTMLElement,
  onReveal: (segments: any, operation: string) => void,
) {
...
export function addAnchorAfterRevealListener(
  element: HTMLElement,
  afterReveal: (
    segments: any[],
    operation: string,
...
export function getAfterReveal(element: HTMLElement) {
  return (
    element["permalink-after-reveal"] ||
    ((segments, operation, leafElement) => {})
  );
}
