        ...
        for (let i = this.line_offset; i < this.tokens.length; i++) {
            ...
            for (let token of line) {
                ...
                if (token_end < start) {
                    ...
                    result.push(<span key={'highlighted-token-' + result.length} style={this.style(token.variants[0])}>{token_content}</span>);
...
function createSharedHighlighter(): Promise<any> {
    ...
    return new Promise((resolve, reject) => {
        ...
        if (handle) {
            ...
            createHighlighter({ themes: ['github-light'], langs: ['markdown', 'python', 'js', 'typescript', 'bash', 'css'] }).then((highlighter: any) => {
...
class CodeHighlightedView extends React.Component<CodeHighlightedViewProps, { nodes: any, content: string, tokens: Token[][], language: string }> {
    constructor(props) {
        ...
        this.state = {
            ...
            tokens: [],
            language: LANGUAGE_CLASSIFIER.derive_highlighting_language(props.content) as string
        };
    }
    ...
    async updateContent(tokens: Token[][]) {
        ...
        let highlights_in_text = this.props.highlights.in_text(JSON.stringify(content, null, 2))
    ...
    render() {
        return <div className='plugin code-highlighter'>
    ...
    derive_highlighting_language(content: string, return_distribution = false): string | Record<string, number> {
        ...
        if (max - second_max < 0.05 * total) {
            if (return_distribution) {
                return Object.fromEntries(Object.entries(counts).map(([key, value]) => [key, Math.round(value * 100) / 100]));
            }
            return 'plaintext';
        }
