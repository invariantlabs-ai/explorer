...
function NewAPIKeyModal({ apiKey, onClose }) {
  ...
  return (
    <div className="form" style={{ maxWidth: "500pt" }}>
      ...
      <h2>
        After closing this dialog, you will not be able to see the key again.
      </h2>
      <input
        type="text"
        value={apiKey}
      ...
      >
        {justCopied ? "Copied to clipboard!" : "no"}
      </span>
...
export function Settings() {
  ...
  return (
    <div className="panel entity-list">
      {createdApiKey && (
        <Modal title="Your API Key" onClose={() => setCreatedApiKey(null)}>
          <NewAPIKeyModal
      ...
      <header>
        <h2 className="home">Settings</h2>
        <div className="spacer" />
        <div className="actions">{/* no actions on top level */}</div>
      </header>
      <div className="box">
        <h2>
          API Keys
          ...
          <button className="inline primary" onClick={() => onCreateAPIKey()}>
            New API Key
          </button>
        </h2>
        <table className="data api-keys">
          <tbody>
            {keys.map((apikey: any) => (
              <tr className={apikey.expired ? "expired" : ""} key={apikey.id}>
                <td>
                  <BsKey />{" "}
                  <code>***************{apikey.hashed_key.substr(0, 8)}</code>
                </td>
                <td>Created on {apikey.time_created}</td>
                <td className="actions">
                  <button
                    className="tool"
                    disabled={apikey.expired}
            ...
            {keys.length == 0 && <div className="empty">No API Keys</div>}
          </tbody>
        </table>
      </div>
    </div>
