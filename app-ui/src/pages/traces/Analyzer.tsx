 ...
 * Sends an input list (list of strings), options and context.
 * Calls progressCallback for every progress update received.
 */
export function createAnalysisJob(
  ...
  endpoint = endpoint + "/api/v1/analysis/job";
  const body = JSON.stringify({ input, options, context });
  const headers = {
...
/**
 * Retrieves the progress of an existing job.
 * Calls progressCallback for every progress update received.
 */
export function getAnalysisJobProgress(
  jobId: string,
  token: string,
  progressCallback: (progress: any) => void,
  errorCallback: (error: any) => void
): void {
  const endpoint = `/api/v1/analysis/job/${jobId}`;
