...
async def test_highlighted_tool_arg(context, url, data_code, screenshot):
    async with util.TemporaryExplorerDataset(url, context, data_code) as dataset:
        ...
        await (
            second_message.get_by_role("row", name="plan Formatted // First, get")
            .get_by_role("button")
            .click()
        )
...
async def test_remove_line_numbers(context, url, data_line_numbers, screenshot):
    async with util.TemporaryExplorerDataset(
        ...
        assert is_highlighted(
            snippet_1_screenshot
        ), "Code shown in table should be highlighted"
        assert is_highlighted(
            snippet_2_screenshot
        ), "Code shown in table should be highlighted"
...
def is_highlighted(screenshot):
    ...
    for x in range(img.width):
        for y in range(img.height):
            ...
            grey_distance = (
                abs(pixels[x, y][0] - pixels[x, y][1])
                + abs(pixels[x, y][1] - pixels[x, y][2])
                + abs(pixels[x, y][2] - pixels[x, y][0])
            )
            if grey_distance > 10:
